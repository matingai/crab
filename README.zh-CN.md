...
- [Agent Loop Eval](docs/AGENT_LOOP_EVAL.md)：用于验证主模型/小模型路由、工具 payload 和
  grounded loop 行为的 CLI 评测。
...
- **智能小模型执行层**：简单回合可以切到更便宜的小模型，并且不发送完整工具 schema；主模型则保留给目标判断、
  取舍决策和需要工具证据的复杂工作。
...
可选的智能路由可以把复杂、需要工具证据的工作留给主模型，把简单回合交给更便宜的小模型：
...
- 原生 computer-use 准备能力：macOS Accessibility 状态、授权弹窗、带紧凑状态标记和非敏感元数据的前台
  ...
  与前台 app 就绪等待、写动作前 ref 和前台 app guard、native action 可用性检查、校验新鲜度、边界和
  前台 origin 的绑定 snapshot focus/click/perform_action/set_text/scroll/key 写动作、干净的写动作后
  post snapshot id 与元数据、白名单原生 Accessibility action 和白名单按键。
