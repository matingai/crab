...
`click` 和 `set_text` 都是写动作。即使用户没有配置自定义 `tool_policy`，Crab 默认也会在执行前要求
approval。只读 action 仍然可以直接运行。`set_text` 不发送全局 keystroke，而是尝试直接设置目标
Accessibility element 的 value，因此主要适合文本框和类似控件。
