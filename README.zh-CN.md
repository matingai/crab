...
Hermes Agent RS 的核心不是单一聊天界面，而是一个本地 agent runtime。CLI、桌面壳、工具、
...
agent loop 是这个项目真正的中心。Hermes Agent RS 把主 agent 设计成一个面向目标求解的控制模型，
...
运行时会从 CLI 参数、环境变量和本地 data directory 中解析配置。默认 data directory 为：
...
仓库目前还没有选择开源许可证。公开发布前请添加 `LICENSE` 文件，让下游用户明确知道授权范围。
