...
The easiest way to try Crab is to download a desktop installer from
[GitHub Releases](https://github.com/matingai/crab/releases). Tagged releases are built on
native GitHub-hosted runners, so users do not need Rust, Node.js, or a local build
environment.

| Platform | Release asset | Install experience |
| --- | --- | --- |
| macOS Apple Silicon | `crab-desktop-vX.Y.Z-aarch64-apple-darwin.dmg` | Open the DMG, then drag Crab into Applications. |
| macOS Intel | `crab-desktop-vX.Y.Z-x86_64-apple-darwin.dmg` | Open the DMG, then drag Crab into Applications. |
| Windows x64 | `crab-desktop-vX.Y.Z-x86_64-pc-windows-msvc-setup.exe` | Run the setup installer. |

Each installer is published with a matching `.sha256` checksum and `.json` manifest. The
current 0.1.x builds are unsigned preview installers, so macOS Gatekeeper and Windows
SmartScreen may show warnings until code signing and notarization are added. See
[Desktop Packaging](docs/DESKTOP_PACKAGING.md) for the release pipeline and signing plan.
...
- Native computer-use preparation through macOS Accessibility status, permission prompt,
  ...
  bounds-checked snapshot-bound focus/click/perform_action/set_text/scroll/key actions
  with post-action snapshot ids, whitelisted native Accessibility actions, and whitelisted
  key presses.
