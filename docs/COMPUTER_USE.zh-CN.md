...
| `wait_app` | 轮询 snapshot，直到前台应用名称或 pid 匹配 `expect_app` 或 `expect_pid`。 |
...
`wait`、`wait_app`、`wait_ref` 和 `inspect_ref` 都会返回同一组 `snapshot_*` 元数据头，让 agent 可以在观察和行动
...
`wait_app` 是只读的前台应用等待循环。用户或系统可能把焦点切走时，agent 可以先等待目标 app 或 pid
回到前台，再请求 approval 执行写动作：
...
在执行前要求 approval。`status`、`snapshot`、`inspect_ref`、`find`、`wait`、`wait_app` 和 `wait_ref`
仍然可以不经 approval 直接运行。
